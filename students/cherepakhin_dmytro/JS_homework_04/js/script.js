...
function getTime(seconds) {
    ...
    if ((seconds >= 0)) {
        ...
        timeText = showTime(hour, minute, second);
        return timeText;
    } else {
        return 'Incorrect value of time'
    }
...
function getDifference(hour1, minute1, second1, hour2, minute2, second2) {
    if (makeValidation(hour1, minute1, second1) && makeValidation(hour2, minute2, second2)) {
        let differenseInSeconds = Math.abs(getTimeInSeconds(hourEnd, minuteEnd, secondEnd) - getTimeInSeconds(hourStart, minuteStart, secondStart));
